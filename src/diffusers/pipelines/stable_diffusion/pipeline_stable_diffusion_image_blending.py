...
class StableDiffusionImageBlendingPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        self.check_inputs(image1, height, width, callback_steps)
        ...
        image_embeddings1 = self._encode_image(image1, device, num_images_per_prompt, do_classifier_free_guidance)
        ...
        #print(image_embeddings1[0,:,:].mean())
        #3.a Blend images
        ...
        if anchor_img:
            #print(latents.shape)
            encoded=self.vae.encode(torch.nn.functional.interpolate(pil_to_tensor(image1).cuda().unsqueeze(0)/255, (512,512))).latent_dist.mode()
            #print (encoded.shape)
            latents=latents*(1-anchor_img)+encoded*anchor_img
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                encoder_hidden_states=[image_embeddings1,image_embeddings2][i%2]).sample
